...
function makeCursorPanel(cursor) {
  const panel = document.createElement('div');
  panel.className = 'placementPanel';
  panel.innerHTML = '<b>配置カーソル</b><span data-cursor-status></span><div><button data-move="0,-1">↑</button></div><div><button data-move="-1,0">←</button><button data-rotate>回転</button><button data-move="1,0">→</button></div><div><button data-move="0,1">↓</button></div>';
...
function boot() {
  ...
  if (debugEnabled()) {
    const islandB = layout.islands.find((item) => item.id === 'island-b');
    let panel = null;
    const cursor = createPlacementCursor(layout, islandB, ({ item, result }) => {
      renderFoundationDebug(scene, report, result);
      if (panel) {
        const label = panel.querySelector('[data-cursor-status]');
        label.textContent = `${item.x},${item.y} ${item.orientation} / ${result.ok ? 'OK' : 'NG'}`;
    ...
    renderFoundationDebug(scene, report, initial.result);
    const label = panel.querySelector('[data-cursor-status]');
    label.textContent = `${initial.item.x},${initial.item.y} ${initial.item.orientation} / ${initial.result.ok ? 'OK' : 'NG'}`;
    scene.dataset.foundationDebug = 'on';
  }

  if (status) {
    status.textContent = `GRID OK / ${report.itemCount} objects / ${report.reachableCells} walk / ROT 4`;

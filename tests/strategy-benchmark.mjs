...
function breaksChain(s,r,R,exitMode){const roll=R();if(exitMode==='signal')return !!s.chain&&s.chain!==r.signal;return roll<.5}
function step(s,r,R,slope,exitMode='random50'){const n={...s};n.energy-=Math.min(n.energy,r.cost);n.depth++;let g=r.gain;if(r.tone==='res'){if(n.chain===r.signal){n.chainLen++;g+=1+n.chainLen*slope}else{n.chain=r.signal;n.chainLen=1}}else if(breaksChain(n,r,R,exitMode)){n.chain=null;n.chainLen=0}if(r.anomaly)g+=anomalyBonus(n.depth);n.haul+=g;n.threat=threatAfter(s,r);if(R()*100<n.threat)return{...n,alive:false,haul:0};if(n.energy<=0)return{...n,alive:false,bank:bankFor(n.haul,n.depth),forced:true};return n}
...
function chainPreserveProbability(s,r,exitMode){if(r.tone==='res')return 1;if(!s.chain)return 0;if(exitMode==='signal')return s.chain===r.signal?1:0;return .5}
function chainAwareChoice(s,rs,slope,exitMode){let best=0,bv=-Infinity;for(let i=0;i<rs.length;i++){const r=rs[i];const base=immediateGain(s,r,slope)-r.cost*.65-threatAfter(s,r)*.055;let postLen=0;if(r.tone==='res')postLen=s.chain===r.signal?s.chainLen+1:1;else postLen=s.chainLen;const preserve=chainPreserveProbability(s,r,exitMode);const nextResContinuation=postLen>0?preserve*(1/3)*(1+(postLen+1)*slope):0;const v=base+nextResContinuation;if(v>bv){bv=v;best=i}}return best}
function choose(policy,s,rs,slope,exitMode='random50'){if(policy.startsWith('fixed:'))return rs.findIndex(r=>r.tone===policy.slice(6));if(policy==='one-step')return oneStepChoice(s,rs,slope);if(policy==='chain-aware')return chainAwareChoice(s,rs,slope,exitMode);return futureChoice(s,rs,slope)}
...
console.log('\nstructuralProbe=visible-signal-chain-exit productionSlope=2 state-conditioned-extraction');
for(const exitMode of ['random50','signal']){
 console.log(`\nchainExit=${exitMode}${exitMode==='random50'?' production':' candidate'}`);
 for(const policy of [...policies,'chain-aware']){
  const row=aggregate(2,policy,'state-ev',exitMode);
  console.log(policy,row);
 }
}

...
const policies=['fixed:calm','fixed:deep','fixed:res','one-step','bank-ev','future-aware','chain-aware'];
...
function bankEVChoice(s,rs,mode){let best=0,bv=-Infinity;for(let i=0;i<rs.length;i++){const v=oneMoreStepBankEV(s,rs[i],mode);if(v>bv){bv=v;best=i}}return best}
...
function choose(policy,s,rs,mode){if(policy.startsWith('fixed:'))return rs.findIndex(r=>r.tone===policy.slice(6));if(policy==='one-step')return oneStepChoice(s,rs,mode);if(policy==='bank-ev')return bankEVChoice(s,rs,mode);if(policy==='chain-aware')return chainAwareChoice(s,rs,mode);return futureChoice(s,rs,mode)}

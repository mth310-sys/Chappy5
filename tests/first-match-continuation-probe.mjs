...
//   noFirstMatchBonus: ONLY that first forced step suppresses the continuation bonus
// All later steps, chain state, costs, Threat, anomalies, extraction, chain-break semantics,
...
function step(s,r,breakU,collapseU,suppressFirstMatchBonus=false){const n={...s};n.energy-=Math.min(n.energy,r.cost);n.depth++;let g=r.gain;if(r.tone==='res'){
 if(n.chain===r.signal){n.chainLen++;if(!suppressFirstMatchBonus)g+=resBonus(n.chainLen)}
...
function branch(start,first,t,h,mode){let s={...start,alive:true,collapsed:false};for(let i=0;i<h;i++){const e=t[i],r=i?policy(s,e.rs):first;if(i&&extract(s,r))return bank(s.haul,s.depth);const suppress=mode==='noFirstMatchBonus'&&i===0&&s.chainLen===1&&r.tone==='res'&&s.chain===r.signal;s=step(s,r,e.breakU,e.collapseU,suppress);if(!s.alive)return s.collapsed?0:bank(s.haul,s.depth)}return bank(s.haul,s.depth)}
const buckets=()=>({2:{sw:0,res:0,tie:0,d:0},3:{sw:0,res:0,tie:0,d:0}});
function sample(seed,mode){const o={samples:0,h:buckets()};for(let k=0;k<RUNS;k++){const R=rng(mix(seed,k));let s={energy:10,depth:0,haul:0,threat:6,chain:null,chainLen:0,alive:true,collapsed:false},visit=0;while(s.alive){const rs=routes(R),res=rs.find(x=>x.tone==='res');if(s.chain&&s.chainLen===1&&res.signal===s.chain){o.samples++;visit++;const non=rs.filter(x=>x.tone!=='res').sort((a,b)=>oneEV(s,b)-oneEV(s,a))[0];for(const h of H){const t=tape(mix(seed,k,visit*17+h),h),rv=branch(s,res,t,h,mode),nv=branch(s,non,t,h,mode),d=nv-rv,b=o.h[h];if(d>0)b.sw++;else if(d<0)b.res++;else b.tie++;b.d+=d}}if(extract(s,res))break;s=step(s,res,R(),R(),false)}}return o}
...
const production=report('production'),noFirstMatchBonus=report('noFirstMatchBonus');
console.log({production,noFirstMatchBonus,deltaNoFirstMatchBonusVsProduction:delta(noFirstMatchBonus,production)});

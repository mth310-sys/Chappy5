...
function firstMatchOverride(mode,i,s,r){if(i!==0||s.chainLen!==1||r.tone!=='res'||s.chain!==r.signal)return null;if(mode==='firstMatchBonus3')return 3;if(mode==='noFirstMatchBonus')return 0;return null}
function branch(start,first,t,h,mode){let s={...start,alive:true,collapsed:false};for(let i=0;i<h;i++){const e=t[i],r=i?policy(s,e.rs):first;if(i&&extract(s,r))return bank(s.haul,s.depth);s=step(s,r,e.breakU,e.collapseU,firstMatchOverride(mode,i,s,r));if(!s.alive)return s.collapsed?0:bank(s.haul,s.depth)}return bank(s.haul,s.depth)}
...
const production=report('production'),firstMatchBonus3=report('firstMatchBonus3'),noFirstMatchBonus=report('noFirstMatchBonus');
console.log({production,firstMatchBonus3,noFirstMatchBonus,deltaBonus3VsProduction:delta(firstMatchBonus3,production),deltaNoFirstMatchBonusVsProduction:delta(noFirstMatchBonus,production)});

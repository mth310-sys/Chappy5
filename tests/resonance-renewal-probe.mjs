...
function stats(){return{samples:0,mismatchSamples:0,h:{2:{sw:0,res:0,tie:0,d:0},3:{sw:0,res:0,tie:0,d:0}},mismatch:{h:{2:{sw:0,res:0,tie:0,d:0},3:{sw:0,res:0,tie:0,d:0}}}}}
function sample(seed,mode){const o=stats();for(let k=0;k<RUNS;k++){const R=rng(mix(seed,k));let s={energy:10,depth:0,haul:0,threat:6,chain:null,chainLen:0,alive:true,collapsed:false},visit=0;while(s.alive){const rs=routes(R),res=rs.find(x=>x.tone==='res');if(s.chain){o.samples++;visit++;const non=rs.filter(x=>x.tone!=='res').sort((a,b)=>oneEV(s,b)-oneEV(s,a))[0];const mismatch=res.signal!==s.chain;if(mismatch)o.mismatchSamples++;for(const h of H){const t=tape(mix(seed,k,visit*17+h),h),rv=branch(s,res,t,h,mode),nv=branch(s,non,t,h,mode),d=nv-rv;const bucket=o.h[h];if(d>0)bucket.sw++;else if(d<0)bucket.res++;else bucket.tie++;bucket.d+=d;if(mismatch){const m=o.mismatch.h[h];if(d>0)m.sw++;else if(d<0)m.res++;else m.tie++;m.d+=d}}}if(extract(s,res))break;s=step(s,res,R(),R(),'production')}}return o}
function merge(rows){const o=stats();for(const r of rows){o.samples+=r.samples;o.mismatchSamples+=r.mismatchSamples;for(const h of H){for(const k of ['sw','res','tie','d']){o.h[h][k]+=r.h[h][k];o.mismatch.h[h][k]+=r.mismatch.h[h][k]}}}return o}
const pct=(a,b)=>b?100*a/b:0;
function report(mode){const s=merge(SEEDS.map(x=>sample(x,mode)));return{mode,activeChainSamples:s.samples,mismatchSamples:s.mismatchSamples,mismatchPct:pct(s.mismatchSamples,s.samples),all:Object.fromEntries(H.map(h=>[h,{switchWinsPct:pct(s.h[h].sw,s.samples),resonanceWinsPct:pct(s.h[h].res,s.samples),tiePct:pct(s.h[h].tie,s.samples),meanSwitchMinusResBank:s.h[h].d/s.samples}])),mismatch:Object.fromEntries(H.map(h=>[h,{switchWinsPct:pct(s.mismatch.h[h].sw,s.mismatchSamples),resonanceWinsPct:pct(s.mismatch.h[h].res,s.mismatchSamples),tiePct:pct(s.mismatch.h[h].tie,s.mismatchSamples),meanSwitchMinusResBank:s.mismatch.h[h].d/s.mismatchSamples}]))}}
...
console.log({production,noRenew,deltaNoRenewVsProduction:{all:delta(noRenew,production,'all'),mismatch:delta(noRenew,production,'mismatch')}});

...
qaResetBehaviorLastUpdated: 2026-09-13

## releaseDateEvidence
- ALL7.jpの2007年7月導入予定一覧は `幻獣覇王 / アイジーティー / 2007-07-09` と掲載。
- HAZUSEの当時解析は `<2007年5月>` 表記で、作成日2007-05-15・更新日2007-07-10。解析ページ作成/掲載月と実導入日の混同余地があるため、導入日根拠としては弱い。
...
- RTの1Gあたり純増については表記揺れ・型式名・メーカー名と `巫女チャンス / RT / 純増` を変えて再探索したが比較可能な高信頼数値を確定できずUNVERIFIED。

## basicPayout
資料間で平均獲得表記に差があるためCONFLICT保持。
- HAZUSE: 巫女ボーナス466枚超払い出し終了・平均約370枚 / 兄様270枚超・平均約210枚 / 幻獣12Gまたは8回入賞・平均約100枚。
- パチマガスロマガ旧ページ: 巫女規定払い出し456枚表記・純増約360枚 / 兄様270枚・純増約260枚 / 幻獣12Gまたは8回入賞・純増約90枚。
- 中古実機資料: 型式 `ゲンジュウハオウT`、巫女466枚以上・平均約370枚 / 兄様270枚以上・平均約210枚 / 幻獣8回入賞または12G・平均約100枚。
- 平均/純増/払い出し終了条件の定義を混同せずCONFLICTとして保持。
信頼度: ANALYSIS_HIGH / CONFLICT
...
resetBehaviorQADate: 2026-09-13

### settingChangeBehavior
- `幻獣覇王 / 幻獣覇王T / ゲンジュウハオウT / IGTジャパン` と `設定変更 / リセット / 朝一 / RT / 巫女チャンス / ガックン / 設定キー / キャパシタ` を組み替え、当時解析・旧DB・中古実機資料・後年回顧資料を再探索。
- 本機固有の設定変更時RT残G・成立済みボーナス・内部状態処理を固定できる高信頼公開資料は確認できず `UNVERIFIED_AFTER_RESEARCH`。

### carryOverBehavior
- 据え置き時のRT残G、成立済みボーナス、その他内部状態の具体的引継ぎを示す本機固有資料は `UNVERIFIED_AFTER_RESEARCH`。

### powerCycleBehavior
- 電源OFF→ONのみの場合にRT残Gや内部状態を引き継ぐ/初期化するかを示す本機固有の公開仕様は `UNVERIFIED_AFTER_RESEARCH`。
- 2026年の中古実機出品に「キャパシタは切れているので設定変更から遊ぶかキャパシタ交換」との実機個体情報を確認したが、これは個体のバックアップ部品劣化を示す販売者記述であり、正常個体の純電断契約を確定する根拠には採用しない。

### gameCounterReset
- 通常時のゲーム数天井はNONE_CONFIRMED。
- RTは巫女ボーナス後100G固定だが、設定変更/据え置き/電断時のRT残G処理は `UNVERIFIED_AFTER_RESEARCH`。
...
### stateAfterReset
- 設定変更時のRT/成立済みボーナス等の状態処理は `UNVERIFIED_AFTER_RESEARCH`。

### advantageousSectionReset
- NOT_APPLICABLE（5号機初期・有利区間制度導入前）。

### resetBenefits
- 設定変更/朝一専用の公開恩恵はNONE_CONFIRMED_AFTER_RESEARCH。

### resetPenalties
- 設定変更時のみの公開不利要素はNONE_CONFIRMED_AFTER_RESEARCH。

### resetDetection
- 本機固有のガックン、リール初動、表示等による設定変更/据え置き判別を確定できる高信頼資料は再探索後も `UNVERIFIED_AFTER_RESEARCH`。

### numericResetData
- 朝一当選率、リセット時RT突入率、短縮天井、モード振り分け等の比較可能な公開数値はNONE_CONFIRMED_AFTER_RESEARCH。

## resetQaConclusion
- 性能コア `COMPLETE_CORE` は維持。
- resetBehaviorは資料系統と検索語を変えて遡及再探索したが、設定変更/据え置き/純電断のRT・内部状態契約を機種固有資料で固定できない。
- 一般的な5号機挙動からの推測補完は行わず、`PARTIAL_RESEARCH_EXHAUSTED` としてQAカーソルを先へ進める。
confidence: UNVERIFIED_FOR_MACHINE_SPECIFIC_RESET_CONTRACT / RESEARCH_EXHAUSTED

## sources
取得日（既存性能資料）: 2026-08-31
...
取得日（reset遡及QA）: 2026-09-13
8. 中一商事 — 型式/RT機であることを再確認
   - https://www.nakaiti.com/html/igt002.html
   - reliability: RETROSPECTIVE_SUPPORT
9. Yahoo!フリマ中古実機個体 — キャパシタ劣化時の実機個体記述
   - https://paypayfleamarket.yahoo.co.jp/item/z641187040
   - reliability: ANECDOTAL_DEVICE_EVIDENCE_ONLY / NOT_USED_FOR_NORMAL_RESET_CONTRACT

...
- 差異は平均化せずCONFLICTとして保持。本文表は全設定が揃う当時解析系の必勝本値を採用。
...
- ART/CZ/潜伏中に設定変更した場合の残状態処理は、表記揺れ・型式名・メーカー名と関連語を組み替え、当時解析・旧DB・回顧資料を横断したが直接契約を固定できず `UNVERIFIED_AFTER_RESEARCH`。
信頼度: ANALYSIS_HIGH / PARTIAL_RESEARCH_EXHAUSTED

### carryOverBehavior
- 据え置き時の内部高確/低確、マーベルチャンス、ART残G、潜伏状態の具体的引継ぎを示す本機固有の高信頼公開資料は、再探索後も固定できず `UNVERIFIED_AFTER_RESEARCH`。
信頼度: PARTIAL_RESEARCH_EXHAUSTED

### powerCycleBehavior
- 電源OFF→ONのみで内部高確/RT/CZ/ART状態がどう処理されるかを示す本機固有の高信頼資料は、再探索後も固定できず `UNVERIFIED_AFTER_RESEARCH`。
信頼度: PARTIAL_RESEARCH_EXHAUSTED

### gameCounterReset
- 通常時ゲーム数天井の存在を示す高信頼資料は確認できず、通常時天井ゲーム数は `NONE_CONFIRMED_AFTER_RESEARCH`。
- ARTはゲーム数管理だが、設定変更/据え置き/純電断時の残G処理は `UNVERIFIED_AFTER_RESEARCH`。
信頼度: PARTIAL_RESEARCH_EXHAUSTED
...
### modeAfterReset
- 設定変更後は内部高確スタート。
- 朝一専用の別モード振り分けやモード選択率の公開数値は `PUBLIC_VALUE_NOT_FOUND_AFTER_RESEARCH`。

### stateAfterReset
- 設定変更時: 高確スタートを確認。
- ART/CZ/潜伏中に設定変更した場合の残状態処理は `UNVERIFIED_AFTER_RESEARCH`。
...
### resetPenalties
- 設定変更でART残G等を失うかは再探索後も直接資料を固定できず、公開不利として断定しない。

### resetDetection
- 必勝本は、15枚役取りこぼしを契機とする30G周期の高確移行と特殊リプレイ成立状況を精査することで、設定変更を見抜ける場合があるとしている。
- 本機固有のリールガックン/リール初動による変更判別は、`マーベルヒーローズ / MARVEL HEROES / マーベルヒーローズH / タイヨーエレック` と `設定変更 / リセット / 朝一 / 据え置き / 電源OFF ON / ガックン / リール初動` を組み替えて再探索したが、高信頼資料を固定できず `UNVERIFIED_AFTER_RESEARCH`。
信頼度: ANALYSIS_HIGH / UNVERIFIED_GACKUN_AFTER_RESEARCH

### numericResetData
- 設定変更後: 内部高確スタート（確率ではなく状態確定情報）。
- 高確移行関連: 15枚役取りこぼし契機で30G周期という公開説明あり。
- 朝一○G以内当選率、設定変更時ART当選率、短縮天井数値: `PUBLIC_VALUE_NOT_FOUND_AFTER_RESEARCH`。

### publicMorningNumbers
- 設定変更後: 内部高確スタート。
- 高確移行観察: 15枚役取りこぼし契機の30G周期。
- 朝一専用当選率/短縮天井率/モード振り分け: `PUBLIC_VALUE_NOT_FOUND_AFTER_RESEARCH`。
...
## missingFields
- 通常時50枚あたりゲーム数/ベース: UNVERIFIED_AFTER_RESEARCH
- 据え置き時の内部状態/ART/CZ残状態の具体的引継ぎ: UNVERIFIED_AFTER_RESEARCH
- 電源OFF→ONのみの内部状態/ART/CZ残状態処理: UNVERIFIED_AFTER_RESEARCH
- ART/CZ/潜伏中の設定変更時残状態処理: UNVERIFIED_AFTER_RESEARCH
- 本機固有のガックン/リール初動判別: UNVERIFIED_AFTER_RESEARCH
- 朝一当選率等の公開確率数値: PUBLIC_VALUE_NOT_FOUND_AFTER_RESEARCH

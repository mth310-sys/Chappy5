...
recordNo: 1753
machineName: スマスロ ドルアーガの塔
aliases: Lドルアーガの塔 / ドルアーガ スマスロ / スマスロドルアーガ
...
- HAZUSEは型式 `Lドルアーガの塔ZA`、検定番号 `5S0405`、導入開始日2025-09-08、メーカー ミズホを掲載。
- グリーンべるとの検定通過記事でも `Lドルアーガの塔ZA` を確認。
- G-netはスマスロ `Lドルアーガの塔ZA` とメダル機 `S/ドルアーガの塔/ZU` を別型式として同時展開することを明記。
...
- 通常時ゲーム数天井は非搭載。
- メダル機とは型式・遊技媒体が異なるため独立レコード化。性能値は複数資料が両仕様共通として直接掲載するため共通値を採用し、推測転記ではない。
...
- `電源OFF ON`、`電断`、`電源投入`、正式型式を含めて再探索し、メダル版や他スマスロA+AT機の一般論からは補完していない。
...
- 一部紹介ページがSLOT版/スマスロ版の分類ラベルを混在させるため、型式 `Lドルアーガの塔ZA` と検定番号 `5S0405` を識別キーとして別仕様を分離した。

## sourceSearchNotes
- resetBehavior再探索語: `スマスロ ドルアーガの塔 設定変更 リセット 朝一 据え置き 電源OFF ON`, `Lドルアーガの塔ZA 電断`, `ドルアーガ ガックン`, `有利区間`, `AT 引き継ぎ`。
- 資料系統: ユニバーサル公式/業界発表、HAZUSE、グリーンべると、G-net、一撃、なな徹、6確、SLOT HACK等。
- メダル版の型式・電源仕様を自動転記せず、共通性能のみ「両仕様共通」と明示した資料から採用。

## sources
retrievedAt: 2026-09-15
- HAZUSE: https://hazuse.com/machine/pachislot/5S0405/
- グリーンべると 検定通過: https://web-greenbelt.jp/post-96513/
- G-net: https://g-net-ps.com/info/s0239/
- 一撃 スマスロ版: https://1geki.jp/slot/l_druaga/

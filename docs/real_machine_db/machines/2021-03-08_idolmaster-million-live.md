...
## identity / release
- SANKYO公式オンライン博物館で販売名「パチスロ アイドルマスター ミリオンライブ！」、型式 `SアイドルマスターG`、ブランド ビスティ、導入年月2021年3月を確認。
- 当時攻略資料・P-WORLDで全国導入開始日2021-03-08を照合。
- 検定番号は型式名・メーカー・検定情報を再探索したが、信頼できる公開値を固定できずUNVERIFIED。
...
| 5 | 1/221.1 |
| 6 | 1/196.6 |
- SANKYO公式、パチ＆スロ必勝本系、複数攻略DBで一致。
- reliability: OFFICIAL / ANALYSIS_HIGH

## baseGamesPer50
- 約38.8G/50枚。
- なな徹の小役解析、ちょんぼりすた、すろぱちくえすとで一致。
- reliability: ANALYSIS_HIGH_MULTI_SOURCE

## netIncrease
- AT「シアターフェスティバル」: 約0.3枚/G。
- 上位状態「シアターフェスティバルBOOST」: 約4.3枚/G。
- SANKYO公式はAT機として掲載し、当時攻略資料も純増0.3 or 4.3枚/Gで一致。
- reliability: OFFICIAL / ANALYSIS_HIGH
...
- reliability: OFFICIAL / INDUSTRY_DATABASE_HIGH

## modeSpecificMinimumData
- 通常時は毎ゲームファン数を獲得し、規定ファン数でCZ抽選。
- ファン数9999人到達で次回CZ成功濃厚（AT到達保証側）。
- CZスルー天井は資料表現に差があり、「CZ8連続スルー」と「4〜9回のシナリオ最終CZ」の表現を確認。単一固定回数へ平均化しない。
- 別系統資料では通常時900Gのゲーム数天井（擬似ボーナス当選）も報告されるが、主要解析DBでは当初未公表/存在示唆に留まるため `ANALYSIS_SINGLE_LATE_DISCLOSURE` として補助保持。
- ファン数は有利区間ごとにリセットされる。
- 有利区間移行時のモード振り分け公開値:
  - 設定1: A 47.1% / B 26.8% / チャンス25.7% / ブースト0.4%
  - 設定2: A 23.0% / B 46.4% / チャンス30.2% / ブースト0.4%未満相当（掲載表の端数/空欄は推定補完しない）
  - 設定3: A 39.7% / B 32.9% / チャンス27.1% / ブースト残余を推定しない
  - 設定4: A 33.0% / B 38.3% / チャンス29.3% / ブースト残余を推定しない
  - 設定5: A 27.9% / B 42.4% / チャンス29.3% / ブースト残余を推定しない
  - 設定6: A 22.1% / B 47.1% / チャンス30.4% / ブースト残余を推定しない
- 上記モード表は「有利区間移行時」であり、設定変更専用表とは断定しない。
...
- 有利区間移行時はモードA/B/チャンス/ブーストの抽選が行われ、設定別公開表あり。

### carryOverBehavior
- 据え置きで有利区間中のままなら、公開朝一資料の電源ON/OFF契約に基づき **全ての状態を引き継ぐ** と扱う。
- ただし店側が閉店後に設定変更等を行ったケースは別。

### powerCycleBehavior
- 設定変更なしの電源OFF→ONで、有利区間中（有利区間ランプ点灯時）は **全ての状態を引き継ぐ**。
- 非有利区間中に電源OFF→ONした場合は、設定変更時と同様の挙動。
- reliability: ANALYSIS_HIGH（なな徹の機種固有朝一ページ）

### gameCounterReset
- 本機の主要進行値は単純なG数だけでなく、ファン数/CZシナリオ/有利区間内状態を含む。
- 設定変更: 非有利区間開始のため前有利区間の進行をRESET側として扱う。
- 据え置き・純電断: 有利区間中ならCARRY_OVER。
- 通常時900G天井については後発単一系統情報のため、設定変更時の個別カウンタ契約を独立確定値として追加しない。
...
### modeAfterReset
- 有利区間移行時のモードA/B/チャンス/ブースト設定別振り分けを公開資料で確認。
- これは「設定変更時だけ」の表ではなく有利区間移行時共通表として保存。
...
- 設定変更: RESET → 非有利区間開始。
- 据え置き/純電断: 有利区間中ならCARRY_OVER。
- ミリオンチャンス後アンコール失敗等で通常遊技中に有利区間が切れる仕様もあるため、朝一ランプは閉店時状態・店側処理を考慮する。

### resetBenefits
- すろぱちくえすとは「リセット後の恩恵: なし」「リセット狙い: なし」と整理。
- 一方、有利区間再移行時にモード抽選を受けられ、開始1G/3Gの示唆を確認できる実戦上の情報価値はある。
...
- 有利区間移行時モード振り分け: 設定1〜6のA/B/チャンス/ブースト表あり（上記modeSpecificMinimumData参照）。
- 通常仕様のベース: 約38.8G/50枚（朝一専用値ではない）。
...
1. SANKYOオンライン博物館「パチスロ アイドルマスター ミリオンライブ！」
   ...
   - 公式の販売名、型式SアイドルマスターG、ビスティ、導入年月、ATゲーム性、設定別初当たり。
...
5. なな徹「規定ファン数テーブル」
   ...
   - 9999人天井、有利区間移行時の設定別モードA/B/チャンス/ブースト振り分け。
   - reliability: ANALYSIS_HIGH

6. すろぱちくえすと「天井解析・朝一設定変更」
   - https://www.slopachi-quest.com/article/mirimasu-tenjou/
   - ファン9999人/CZスルー天井、38.8G/50枚、リセット恩恵なし、ランプ点灯据え置き濃厚/消灯リセット濃厚。
   - reliability: ANALYSIS_PERIOD

7. P-WORLD「パチスロ アイドルマスター ミリオンライブ！」

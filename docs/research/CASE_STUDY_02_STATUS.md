...
> Contrast: Case Study 01「スマスロ北斗の拳」
...
- `CURRENT STOP CAN BE THE CONTENT`
- `MASTERY SHOULD CHANGE PERCEPTION`
- `QUESTION DENSITY PER ORDINARY GAME`
- `REPEATABILITY = STABLE ROUTINE × VARIABLE INTERPRETATION × LEARNABLE RESOLUTION`
- `REWARD CAN BE DISCOVERED BEFORE ANNOUNCED`
- `CHANGE RESPONSIBILITY BEFORE ADDING CONTROLS`
- `BASELINE STABILITY CREATES DEVIATION VALUE`
- `SKILL REWARD SHOULD SCALE PRIDE FASTER THAN REGRET`
...
1. Stable body grammar anchors repeated play while meaning changes.
2. Strong deviations require a trusted normal baseline in timing/light/sound.
3. Input/role value is contextual rather than fixed by presentation alone.
4. Player interpretation is part of the game, whether reading future-state expectation or present reel relations.
5. Reward needs sustain quality, not only entry impact.
...
No original-concept refinement, code, prototype, visual/audio asset, cabinet/mechanism implementation or reel implementation before explicit `HUMAN_MACHINE_APPROVED`.
# Case Study 03 — Current Handoff

> Target: メーシー「SLOT魔法少女まどか☆マギカ2」（2016)
> Lineage: `A+ART / BONUS + ART / CZ / game-count extension`
> Current phase: `PHASE A — REAL MACHINE STUDY / RUN 3`
> Stage: `Stage 1 — Reality Baseline Study`
> Human Gate: `HUMAN_GATE_1_NOT_READY`
> Reality Baseline: `NOT_COMPLETE`
> Production: `DENIED_UNTIL_HUMAN_MACHINE_APPROVED`

## Formal state

- Case Study 01 スマスロ北斗の拳: current five-Director cycle complete.
- Case Study 02 新ハナビ: formally opened after Case Study 01 and its current research cycle is complete; do not regress to it as though it were unopened.
- Case Study 03 remains the current shared object.
- Case Study 03 Phase A real-machine Run 1: complete and Machine-integrated.
- Case Study 03 Phase B smartphone-app Run 1: complete and Machine-integrated, retained only as a source of questions.
- Case Study 03 Phase A real-machine Run 2: all specialist Directors complete and Machine-integrated.
- Standing HUMAN DIRECTIVE: `REMAIN IN REAL-MACHINE STUDY; DO NOT RETURN TO ORIGINAL CONCEPTS.`
- Therefore `PHASE C — REAL↔APP TRANSLATION` remains NOT OPEN.
- Machine Director formally opens `PHASE A — REAL MACHINE STUDY / RUN 3` as a targeted unresolved-evidence pass.

## Run 2 Machine integration

Read:
`docs/research/CASE_STUDY_03_MADOKA_MAGICA_2_MACHINE_INTEGRATION_RUN2.md`

Integrated loop:

`TRUSTED HOME ROUTINE → ROLE/STATE QUESTION → PHYSICAL REEL EVIDENCE → CONSEQUENCE WINDOW → LOCAL REWARD OR PERSISTENT-STATE EDIT → RE-ANCHOR → TRUSTED HOME ROUTINE`

Cross-Director conclusions:
- `BODY GRAMMAR CAN STAY STABLE WHILE RESPONSIBILITY GRAMMAR CHANGES`;
- `ROLE CLASSIFICATION LATENCY ≠ CONSEQUENCE-RESOLUTION LATENCY`;
- reel/task hierarchy is better judged by re-acquisition cost than display area alone;
- `CERTAINTY TIMING IS AN EXPERIENCE PARAMETER`;
- local BONUS reward must be evaluated before downstream ART value is counted;
- base ART must retain value before exceptional extension states are counted;
- a state return is incomplete until state, attention, action responsibility, audible context and remaining-value understanding are re-anchored;
- three-hour fatigue is attention economics, not merely motor repetition.

Machine verdict: available evidence supports a coherent HOME-and-return architecture, but does not yet quantify its failure boundary over long low-setting/dry play. Run 2 is complete as a research pass; the overall Reality Baseline is not complete.

## Run 3 shared target

`MEASURE / TRIANGULATE THE HOME-RETURN FAILURE BOUNDARY.`

Run 3 must be narrower than prior passes and attack unresolved evidence rather than repeat general machine description.

### Game & Reel Director — NEXT
- quantify, where reliable sources permit, how often normal play branches away from the low-cost routine;
- map questions that close immediately vs after a short observation window vs remain statistical only;
- distinguish mandatory player duty from optional expert inference;
- never invent undisclosed stop-control tables.

### Visual & Mechanism Director
- timestamp long-form real-machine footage for gaze-demand/state-density samples where feasible;
- test reel/operation-anchor re-acquisition after LCD-heavy events;
- keep documented hardware separate from inferred depth/material observations.

### Sound & Experience Director
- sample repeated long-form footage for cue density and REST/silence intervals rather than memorable cues only;
- distinguish question-closing audio, question-prolonging audio and HOME-restoring audio;
- do not infer clean assets/SPL/speaker hardware from compressed footage.

### Playtest & QA Director
- reconstruct at least one dry/low-reward sequence and one reward-rich sequence under the same evaluation frame;
- separate `REWARD SATISFACTION` from `RELIEF`;
- track `OPEN-QUESTION COUNT`, `RE-ANCHOR LATENCY` and optional statistical-maintenance burden;
- compare beginner-required duty with expert-chosen observation load.

## Evidence labels

All fixed Directors must separate:
- `SOURCED FACT`
- `DIRECT REAL-MACHINE VIDEO/PHOTO OBSERVATION`
- `FIELD REPORT`
- `INTERPRETATION`
- `REUSABLE PRINCIPLE`
- `UNRESOLVED / CONTRADICTION`

App behavior may generate questions but cannot prove unresolved real-machine properties. Do not copy third-party IP.

## Gate

`CURRENT_SHARED_OBJECT = CASE_STUDY_03`
`CURRENT_PHASE = PHASE_A_REAL_MACHINE_STUDY_RUN3`
`CASE_STUDY_01_CURRENT_CYCLE_COMPLETE = TRUE`
`CASE_STUDY_02_CURRENT_CYCLE_COMPLETE = TRUE`
`CASE_STUDY_03_PHASE_A_RUN2_FIVE_DIRECTOR_SPECIALIST_PASS_COMPLETE = TRUE`
`CASE_STUDY_03_PHASE_A_RUN2_MACHINE_INTEGRATED = TRUE`
`CASE_STUDY_03_PHASE_A_RUN3_OPEN = TRUE`
`NEXT_FIXED_DIRECTOR = GAME_&_REEL`
`PHASE_C_REAL_APP_TRANSLATION = NOT_OPEN`
`HUMAN_GATE_1_NOT_READY`
`REALITY_BASELINE_COMPLETE = FALSE`
`SIGNAL/FORGE/VAULT = PROVISIONAL_FROZEN`
`PRODUCTION = DENIED_UNTIL_HUMAN_MACHINE_APPROVED`
